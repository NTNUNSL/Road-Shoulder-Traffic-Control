...
def choose_action(Q_state,C_flow,Counter,action,Epsilon):
    ...
    if random.uniform(0,1)<Epsilon:
        ...
        for a in action:
            p = float('%.2f'%(math.exp(Q_state[a]/(1000 - Counter[a][C_flow])))) if 1000-Counter[a][C_flow]>0 else float('%.2f'%(math.exp(Q_state[a])))
            Boltz_P.append(p)
            Boltz+=p
        ...
        ##### Boltzmann Distribution #####
        for a in Q_max:
            p = float('%.2f'%(math.exp(Q_state['a%s'%a]/(1000 - Counter['a%s'%a][C_flow])))) if 1000-Counter['a%s'%a][C_flow]>0 else float('%.2f'%(math.exp(Q_state['a%s'%a])))
            Boltz_P.append(p)
            Boltz+=p
        Boltz_P = [a/Boltz for a in Boltz_P]
...
def Update_Q_value(Q_state,Q_now,C_flow,N_state,result,Counter,Qa,Alpha,Gamma):
    ...
    if Q_state.max()==0 or N_state.max()==0:
        QN_state=0
        Q_state[result[1]]=(1-Lr)*Q_state[result[1]]+Lr*(float(result[4])+Gamma*(QN_state))
    else:
        QN_state=0
        N_table=[]
        for s in N_state:
